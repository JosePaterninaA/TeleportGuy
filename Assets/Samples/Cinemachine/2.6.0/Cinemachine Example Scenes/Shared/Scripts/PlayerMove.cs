...
public class PlayerMove : MonoBehaviour
{
    ...
    {
        Plane trailPlane = new Plane(Vector3.up, Fire.transform.position);
        Ray ray = Camera.main.ScreenPointToRay (Input.mousePosition);
        float hitdist = 0.0f;
        if (trailPlane.Raycast (ray, out hitdist)) {
            Vector3 targetPoint = ray.GetPoint(hitdist);
            Vector3 tempRotation = Quaternion.LookRotation(targetPoint - Fire.transform.position).eulerAngles;
            tempRotation.y -= 90;
            Quaternion targetRotation = Quaternion.Euler(tempRotation);
            Fire.transform.rotation = Quaternion.Slerp(Fire.transform.rotation, targetRotation, 30 * Time.deltaTime);
            Debug.DrawLine(Fire.transform.position,targetPoint);
        }
    }
